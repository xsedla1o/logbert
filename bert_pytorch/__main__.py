...
def train():
    ...
    parser.add_argument(
        "-o", "--output_path", required=True, type=str, help="ex)output/bert.model"
    )
    ...
    parser.add_argument(
        "-w", "--num_workers", type=int, default=5, help="dataloader worker size"
    )
    ...
    parser.add_argument(
        "--log_freq", type=int, default=10, help="printing loss every n iter: setting n"
    )
    parser.add_argument(
    ...
    args = parser.parse_args(
        "-c ../data/corpus.small -t ../data/valid.small -v ../data/vocab.small -o ../output/bert.model".split()
    )
    ...
    valid_dataset = (
        BERTDataset(
            args.valid_dataset, vocab, seq_len=args.seq_len, on_memory=args.on_memory
        )
