...
class Predicter:
    ...
    def detect_logkey_anomaly(self, output, label):
        ...
        for i in range(len(label)):
            predicted = (
                torch.argsort(output[i])[-self.num_candidates :].clone().detach().cpu()
            )
    ...
    def find_best_threshold(
        self, test_normal_results, test_abnormal_results, threshold_range
    ):
    ...
    def unsupervised_helper(
        self, model, data_iter, vocab, data_type, scale=None, min_len=0
    ):
        test_results = []
        normal_errors = []
        ...
        with torch.no_grad():
            for idx, line in tqdm(enumerate(data_iter)):
                ...
                result = {
                    "logkey_anomaly": num_logkey_anomaly,
