...
class DeepLog(nn.Module):
    def __init__(
        self, num_labels, hidden_size=100, num_directions=2, topk=9, device="cpu"
    ):
    ...
    def init_hidden(self):
        h0 = torch.zeros(self.num_directions, self.batch_size, self.hidden_size).to(
            self.device
        )
        c0 = torch.zeros(self.num_directions, self.batch_size, self.hidden_size).to(
            self.device
        )
        return (h0, c0)
    ...
    def evaluate(self, test_loader):
        ...
        with torch.no_grad():
            ...
            metrics = {
                "window_acc": accuracy_score(window_y, window_pred),
